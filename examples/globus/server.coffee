...
wls=new Widlib.Server
  pages:
    form:
      ...
      onSubmitServer: (options) ->
        mailOptions =
          from: "robot@interactiff.net"
          to: "ulitiy@gmail.com, denzlobin@gmail.com, estkdostavka@gmail.com"
          subject: "Заказ доставки от #{time()}"
          text: "Адрес: #{options.address}\nТелефон: #{options.phone}\nКомментарий: #{options.comment}\nСумма: #{@sum(options.products)} руб.\n"
        _(options.products).each (product)->
          # здесь нужна проверка цены
          mailOptions.text+="#{product.name} --  #{product.price}руб/шт  |  #{product.quantity} шт.  |  Сумма:#{product.price * product.quantity} руб\n"
